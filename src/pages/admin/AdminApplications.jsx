    ...
    return (
        <AdminLayout>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-black text-slate-800 dark:text-white tracking-tight flex items-center gap-3">
                        <FileText className="text-purple-600 dark:text-purple-500" size={36} />
                        Gestion Candidatures
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Surveillance des flux de recrutement InternFlow</p>
                </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-center">
                <div className="xl:col-span-8 flex bg-white dark:bg-slate-900/40 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800/50 backdrop-blur-sm overflow-x-auto shadow-sm dark:shadow-none">
                ...
                <div className="xl:col-span-4 flex gap-3">
                    <div className="flex bg-white dark:bg-slate-900/40 p-1 rounded-xl border border-slate-200 dark:border-slate-800/50 backdrop-blur-sm shrink-0 shadow-sm dark:shadow-none">
                        <button
                            onClick={() => setViewMode('grid')}
                            className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-slate-100 dark:bg-slate-800 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}
                            title="Vue Carte"
                        >
                            <LayoutGrid size={20} />
                        </button>
                        <button
                            ...
                            className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-slate-100 dark:bg-slate-800 text-purple-600 dark:text-purple-400 shadow-sm' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}
                            title="Vue Liste"
                        >
                            <List size={20} />
                        </button>
                    ...
                    <div className="relative group flex-1">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 group-focus-within:text-purple-500 transition-colors" size={20} />
                        <input
                            type="text"
                            placeholder="Entreprise, candidat, poste..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl py-3 pl-12 pr-4 text-slate-800 dark:text-white focus:outline-none focus:border-purple-500/50 transition-all font-medium placeholder:text-slate-400 dark:placeholder:text-slate-600 shadow-sm dark:shadow-none"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-12 pb-20">
                {loading ? (
                    ...
                    <div className="text-center py-24 bg-white dark:bg-slate-900/20 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                        <FileText size={48} className="mx-auto text-slate-300 dark:text-slate-800 mb-4" />
                        <h3 className="text-xl font-bold text-slate-400 dark:text-slate-400">Aucune candidature trouvée</h3>
                        <p className="text-slate-400 dark:text-slate-500 mt-2">Aucun dossier ne correspond à vos filtres actuels.</p>
                    </div>
                ) : viewMode === 'grid' ? (
                    ...
                    <div className="bg-white dark:bg-slate-900/40 rounded-[2rem] border border-slate-200 dark:border-slate-800/50 overflow-hidden backdrop-blur-sm shadow-sm dark:shadow-none">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-slate-200 dark:border-slate-800/50 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                                        ...
                                        <tr key={app.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden flex-shrink-0">
                                                        {app.applicantPhoto ? (
                                                            ...
                                                            <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-500"><User size={16} /></div>
                                                        )}
                                                    </div>
                                                    <div>
                                                        <p className="font-bold text-slate-800 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{app.applicantName}</p>
                                            ...
                                            <td className="p-6">
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-lg bg-white p-1 flex-shrink-0 border border-slate-100 dark:border-transparent">
                                                        {app.companyLogo ? <img src={app.companyLogo} className="w-full h-full object-contain" /> : <Building2 size={16} className="text-slate-700" />}
                                                    </div>
                                                    <span className="font-bold text-slate-700 dark:text-slate-300">{app.companyName}</span>
                                                </div>
                                            </td>
                                            <td className="p-6">
                                                <span className="font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-900 px-3 py-1 rounded-lg border border-slate-200 dark:border-slate-800">
                                                    {app.jobTitle}
                                                </span>
                                            </td>
                                            ...
                                            <td className="p-6 text-right">
                                                <button
                                                    ...
                                                    className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-white rounded-lg transition-colors"
                                                >
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="w-full md:w-80 bg-slate-50 dark:bg-slate-950/50 border-r border-slate-200 dark:border-slate-800 p-6 sm:p-8 flex flex-col items-center overflow-y-auto md:overflow-visible shrink-0">
                                ...
                                <h2 className="text-xl font-black text-slate-800 dark:text-white text-center">{selectedApp.applicantName}</h2>
                                <p className="text-purple-600 dark:text-purple-500 text-sm font-black uppercase tracking-widest mt-1">Candidat</p>
                            ...
                            <div className="flex-1 flex flex-col overflow-hidden min-h-0 bg-white dark:bg-slate-900">
                                <div className="p-4 sm:p-8 pb-4 flex items-center justify-end border-b border-slate-200 dark:border-slate-800/30 shrink-0">
                                    <button onClick={() => setSelectedApp(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                                        <XCircle size={28} />
                                    </button>
                                </div>
...
function CompanyGroup({ companyName, apps, groupIdx, onOpenApp }) {
    ...
    return (
        ...
        >
            ...
            >
                <div className="w-12 h-12 bg-white dark:bg-white rounded-2xl flex items-center justify-center p-2 shadow-sm dark:shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    {apps[0].companyLogo ? (
                        <img src={apps[0].companyLogo} alt={companyName} className="w-full h-full object-contain" />
                    ) : (
                        <Building2 className="text-slate-400 dark:text-slate-950" size={24} />
                    )}
                </div>
                <h2 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{companyName}</h2>
...
function DetailItem({ icon: Icon, label, value }) {
    return (
        <div className="flex items-start gap-3 group">
            <div className="mt-0.5 p-2 bg-slate-100 dark:bg-slate-900 rounded-xl text-slate-400 dark:text-slate-500 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">
                <Icon size={14} />
            </div>
...
function DocPreview({ label, url, color }) {
    ...
    return (
        <div className={`p-6 ${bgColor} border ${borderColor} rounded-[2rem] flex flex-col items-center justify-center text-center gap-4 group transition-all hover:bg-white dark:hover:bg-slate-900 shadow-sm dark:shadow-none`}>
            <div className={`w-16 h-16 ${iconColor} bg-white dark:bg-slate-950 rounded-[1.5rem] flex items-center justify-center border border-slate-200 dark:border-white/5 shadow-lg dark:shadow-2xl group-hover:scale-110 transition-transform`}>
                <FileText size={32} />
            </div>
            <div>
            ...
            {url ? (
                <a href={url} target="_blank" rel="noreferrer" className={`mt-2 flex items-center gap-2 px-6 py-2 bg-slate-800 dark:bg-white text-white dark:text-slate-950 rounded-xl text-xs font-black uppercase hover:bg-purple-600 dark:hover:bg-purple-500 hover:text-white transition-all shadow-lg`}>
                    Consulter
                </a>
            ) : (
...
function ApplicationCard({ app, delay, onOpen }) {
    return (
        <motion.div
            ...
            whileHover={{ y: -5 }}
            className="group bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/60 rounded-[2.5rem] overflow-hidden shadow-sm dark:shadow-2xl hover:bg-slate-50 dark:hover:bg-slate-900 transition-all hover:border-purple-500/30"
        >
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 p-5 sm:p-6 border-b border-slate-200 dark:border-slate-800/30 relative">
                ...
                <div className="flex items-center gap-4">
                    <div className="w-20 h-20 bg-white dark:bg-slate-950 rounded-2xl p-0.5 border border-slate-200 dark:border-slate-800 shadow-xl relative overflow-hidden group-hover:scale-105 transition-transform">
                        {app.applicantPhoto ? (
                            <img src={app.applicantPhoto} alt={app.applicantName} className="w-full h-full object-cover rounded-xl" />
                        ) : (
                            <div className="w-full h-full flex items-center justify-center text-slate-400 dark:text-slate-700"><User size={32} /></div>
                        )}
                        <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/40 to-transparent"></div>
                    </div>
                    <div>
                        <h4 className="text-[10px] font-black text-purple-600 dark:text-purple-400 uppercase tracking-widest mb-1">Candidat Principal</h4>
                        <h3 className="text-xl font-black text-slate-800 dark:text-white leading-tight group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">{app.applicantName}</h3>
                        <p className="text-xs text-slate-500 font-bold mt-1">
                            Postule pour <span className="text-slate-700 dark:text-slate-300">{app.jobTitle}</span>
            ...
            <div className="p-5 sm:p-6 space-y-4 bg-slate-50 dark:bg-slate-950/20">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <div className="p-2 bg-white dark:bg-slate-900 rounded-xl text-slate-400 border border-slate-100 dark:border-transparent"><Briefcase size={16} /></div>
                        <div>
                            <p className="text-[9px] font-black text-slate-500 uppercase">Division</p>
                            <p className="text-xs font-bold text-slate-700 dark:text-white">Technologie</p>
                        </div>
                    </div>
                    <ChevronRight className="text-slate-400 dark:text-slate-700 group-hover:text-purple-500 translate-x-0 group-hover:translate-x-1 transition-all" size={20} />
                </div>
                ...
                {app.status === 'ACCEPTED' && app.interviewDate ? (
                    ...
                    <div className="p-4 bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl text-center">
                        <p className="text-[9px] font-black text-emerald-600 dark:text-emerald-500 uppercase">En attente de slot</p>
                    </div>
                ) : null}

                <button
                    onClick={onOpen}
                    className="w-full py-3 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-white border border-slate-200 dark:border-slate-800 rounded-2xl text-xs font-black uppercase tracking-widest transition-all shadow-sm dark:shadow-xl active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                >
...
function TabButton({ active, onClick, label, count, color }) {
    const activeColors = {
        all: "bg-white dark:bg-slate-100 text-slate-950",
    ...
    const countColors = {
        all: "bg-slate-100 dark:bg-slate-200 text-slate-600 dark:text-slate-950",
        blue: "bg-blue-800/20 text-blue-100 border border-white/20",
        emerald: "bg-emerald-800/20 text-emerald-100 border border-white/20",
        rose: "bg-rose-800/20 text-rose-100 border border-white/20"
    };
    ...
    return (
        <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all relative ${active
                ? (activeColors[color] || activeColors.all) + " shadow-md"
                : "text-slate-500 dark:text-slate-500 hover:text-slate-700 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/50"
                }`}
        >
            {label}
            {count > 0 && (
                <span className={`px-2 py-0.5 rounded-md text-[10px] ${active ? (countColors[color] || countColors.all) : "bg-slate-200 dark:bg-slate-800 text-slate-500"}`}>
                    {count}
                </span>
            )}
...
function StatusPulse({ status }) {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-white">{cfg.label}</span>
        </div>
    );
}
